...
//                echo "<pre>"; print_r($_POST); exit;
        ...
        if ($id == '') {
            ...
            $data['expense'] = $this->expenses_model->get_expense_data($id);
...
//        echo "<pre>"; print_r($_POST); exit;
        if(!empty($_POST['id'])){
            for($i=0;$i<count($_POST['id']);$i++){
                $data['status'] = 1;
                $data['id'] = $_POST['id'][$i];
                $response = $this->expenses_model->update_approve_status($_POST['id'][$i], $data);
            }
        }
    ...
    public function report(){
        $data['title']     = _l('expense_report_page');
        $data['bodyclass'] = 'expense_report';
        $this->load->view('admin/expenses/report', $data);
    }

    public function show_all_report_filter_data(){
        $response = $this->expenses_model->fetch_report_data($_POST);
